...
for(clust in clusts){
  ...
  for (i in 1:length(df1$Long[sur])){
    upper.half.circle(df1$Long[sur][i], df1$Lat[sur][i], col=colors[df1$Province[sur][i]], r=3)
  }
  for (i in 1:length(df1$Long[dcm])){
    lower.half.circle(df1$Long[dcm][i], df1$Lat[dcm][i], col=colors[df1$Province[dcm][i]], r=3)
  }
